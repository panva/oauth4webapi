...
async function idToken(privateKey: CryptoKey, subject: unknown, hybrid = false) {
...
for (const process of [
  ...
  test(`${process.name}() validates ID Token subject types`, async (testContext) => {
...
for (const [name, validate] of [
  ...
  test(`${name} validates ID Token subject types`, async (testContext) => {
    for (const subject of invalidStrings) {
