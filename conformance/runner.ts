...
export const green = test.macro({
  async exec(t, module: ModulePrescription) {
    ...
    if (usesDpop(variant)) {
      DPoP = <CryptoKeyPair>await jose.generateKeyPair(JWS_ALGORITHM)
    }
    ...
    if (usesPar(plan)) {
      ...
      const params = await oauth.processPushedAuthorizationResponse(as, client, par)
      ...
      authorizationUrl.searchParams.set('client_id', client.client_id)
      authorizationUrl.searchParams.set('request_uri', params.request_uri)
    }
